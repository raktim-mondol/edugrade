...
import { Upload, Users, Brain, Loader2, FileText, CheckCircle, X, BarChart3, Download, ChevronLeft, Plus, LayoutGrid, Trash2, Pencil } from './ui/Icons';
import { Assignment, Section, EvaluatedStudent } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ assignment, onUpdateAssignment, onBack }) => {
  const [activeSectionId, setActiveSectionId] = useState<string | null>(
    assignment.sections.length > 0 ? assignment.sections[0].id : null
  );
  const [isProcessing, setIsProcessing] = useState(false);
  const [progress, setProgress] = useState(0);
  const [selectedStudent, setSelectedStudent] = useState<EvaluatedStudent | null>(null);
  ...
  // Ensure there's at least one section on load if none exist
  useEffect(() => {
    if (assignment.sections.length === 0) {
        const defaultSection: Section = { id: 'default', name: 'Section 1', students: [] };
        onUpdateAssignment({
            ...assignment,
  ...
  const handleDeleteSection = (sectionId: string, e: React.MouseEvent) => {
      ...
      if(confirm("Are you sure you want to delete this section? All student data will be lost.")) {
  ...
  const handleRenameStart = (section: Section, e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const mockNames = ["Alex Chen", "Sarah Jones", "Mike Ross", "Jessica Pearson", "Harvey Specter", "Louis Litt", "Donna Paulsen", "Rachel Zane", "Katrina Bennett", "Harold Gunderson"];
    const mockSubmissions = [
        'The mitochondria is the powerhouse of the cell. It generates ATP through oxidative phosphorylation...',
        'Photosynthesis occurs in the chloroplasts. It converts light energy into chemical energy stored in glucose...',
        'I dont know the answer properly but I think it relates to biology.',
        'Cellular respiration is a set of metabolic reactions and processes that take place in the cells of organisms...',
        'The legal implications of cellular biology are limited, however...',
        'Cats are nice but biology is hard.',
        'The process involves the breakdown of glucose into pyruvate.',
        'ATP synthase is a key enzyme in the process.',
        'The Krebs cycle is also known as the citric acid cycle.',
        'Glycolysis creates a net gain of 2 ATP molecules.'
    ];

    const newStudents: EvaluatedStudent[] = Array.from({ length: 5 }).map(() => {
        const idx = Math.floor(Math.random() * mockNames.length);
        return {
            id: crypto.randomUUID(),
            name: mockNames[idx],
            content: mockSubmissions[idx],
            status: 'pending'
        };
    });
  ...
  const runEvaluation = async () => {
    ...
    for (let i = 0; i < studentsToGrade.length; i++) {
        if (studentsToGrade[i].status !== 'completed') {
            ...
            try {
                const result = await evaluateSubmission(assignment.config, studentsToGrade[i].content);
                studentsToGrade[i].result = result;
          ...
          {!activeSection?.students.length ? (
            <div className="h-full flex flex-col items-center justify-center text-zinc-400 border-2 border-dashed border-zinc-100 rounded-2xl">
              ...
              <label className="cursor-pointer flex items-center gap-2 text-zinc-900 font-medium hover:underline">
                  Upload Files <input type="file" ref={fileInputRef} className="hidden" multiple onChange={handleFileUpload}/>
              </label>
            </div>
          ) : (
                        ...
                        <>
                          <div className="p-8 border-b border-zinc-100 flex justify-between items-start">
                             <div>
                               ...
                               <p className="text-sm text-zinc-400 mt-1 font-mono">ID: {selectedStudent.id}</p>
                             </div>
